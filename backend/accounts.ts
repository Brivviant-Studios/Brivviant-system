...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(b.action==='change_password'){
   ...
   const {error:pe}=await admin.from('bv_profiles').update({must_change:false,uses_default_password:false}).eq('id',user.id);if(pe)throw pe;
   return reply({ok:true});
  }
  if(me.role!=='ceo'||me.must_change)return reply({error:'إدارة الحسابات للـCEO فقط.'},403);
  if(b.action==='create_user'){
   if(!/^[a-z0-9.]{3,40}$/.test(b.username)||!String(b.name||'').trim()||!['ceo','coordinator','employee'].includes(b.role)||!validPassword(b.password))return reply({error:'راجع الاسم واسم المستخدم والصلاحية وكلمة المرور.'},400);
   const {data,error}=await admin.auth.admin.createUser({email:email(b.username),password:b.password,email_confirm:true,app_metadata:{workspace:'brivviant'}});if(error)return reply({error:'اسم المستخدم موجود بالفعل أو البيانات غير صحيحة.'},400);
   const {error:pe}=await admin.from('bv_profiles').insert({id:data.user.id,username:b.username,name:String(b.name).slice(0,100),role:b.role,must_change:true,uses_default_password:false});
   if(pe){await admin.auth.admin.deleteUser(data.user.id);throw pe;}return reply({ok:true});
  }
  ...
  if(b.action==='reset_password'){
   ...
   const {error}=await admin.auth.admin.updateUserById(target.id,{password:b.password});if(error)throw error;
   const {error:pe}=await admin.from('bv_profiles').update({must_change:true,uses_default_password:false}).eq('id',target.id);if(pe)throw pe;
   return reply({ok:true});
  }
  if(b.action==='update_user'){
   if(!['ceo','coordinator','employee'].includes(b.role)||typeof b.active!=='boolean'||!String(b.name||'').trim())return reply({error:'راجع بيانات الحساب.'},400);
   if(target.id===user.id&&(b.role!=='ceo'||!b.active))return reply({error:'لا يمكن تعطيل حسابك الإداري أو إزالة صلاحياتك بنفسك.'},400);
