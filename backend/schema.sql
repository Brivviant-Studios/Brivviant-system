create schema bv_private;
revoke all on schema bv_private from public;
grant usage on schema bv_private to authenticated, service_role;
create table public.bv_profiles (
 id uuid primary key references auth.users(id), username text not null unique check(username ~ '^[a-z0-9.]{3,40}$'),
 name text not null, role text not null check(role in ('ceo','coordinator','employee')), active boolean not null default true,
...
create table public.bv_tasks (
 ...
 brief text not null, drive_url text not null check(drive_url ~ '^https://(drive|docs)\.google\.com/'),
 due_at timestamptz not null, round integer not null default 1, approved_at timestamptz,
 created_by uuid not null references public.bv_profiles(id), created_at timestamptz not null default now(),
 updated_at timestamptz not null default now(), version integer not null default 1
);
create table public.bv_assignments (
 id uuid primary key default gen_random_uuid(), task_id uuid not null references public.bv_tasks(id),
 user_id uuid not null references public.bv_profiles(id), round integer not null, active boolean not null default true,
 status text not null default 'assigned' check(status in ('assigned','working','paused','submitted')),
 elapsed_ms bigint not null default 0 check(elapsed_ms>=0), running_since timestamptz, started_at timestamptz,
 submitted_at timestamptz, submission_url text, delay_reason text, created_at timestamptz not null default now(),
 unique(task_id,user_id,round), check((status='working')=(running_since is not null))
);
create table public.bv_requests (
 id uuid primary key default gen_random_uuid(), created_by uuid not null references public.bv_profiles(id),
 kind text not null check(kind in ('equipment','fault','complaint','other')), title text not null, body text not null,
 status text not null default 'open' check(status in ('open','progress','done')), response text not null default '',
 created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table public.bv_activity (
 id bigint generated always as identity primary key, task_id uuid references public.bv_tasks(id),
 actor uuid not null references public.bv_profiles(id), action text not null, detail text not null default '',
 created_at timestamptz not null default now()
);
create table public.bv_bootstrap (id integer primary key check(id=1), token_hash text not null, expires_at timestamptz not null, used boolean not null default false);
create index bv_assignment_user on public.bv_assignments(user_id,task_id);
create index bv_assignment_task on public.bv_assignments(task_id,round);
create index bv_request_owner on public.bv_requests(created_by);
create index bv_activity_task on public.bv_activity(task_id,created_at);
create index bv_task_creator on public.bv_tasks(created_by);
create index bv_activity_actor on public.bv_activity(actor);
create function bv_private.member_role() returns text language sql stable security definer set search_path='' as $$
 select role from public.bv_profiles where id=auth.uid() and active
$$;
create function bv_private.can_read_task(t uuid) returns boolean language sql stable security definer set search_path='' as $$
...
$$;
revoke all on function bv_private.member_role(), bv_private.can_read_task(uuid) from public;
grant execute on function bv_private.member_role(), bv_private.can_read_task(uuid) to authenticated;
...
alter table public.bv_bootstrap enable row level security;
revoke all on public.bv_profiles, public.bv_tasks, public.bv_assignments, public.bv_requests, public.bv_activity, public.bv_bootstrap from anon, authenticated;
...
grant all on public.bv_profiles, public.bv_tasks, public.bv_assignments, public.bv_requests, public.bv_activity, public.bv_bootstrap to service_role;
grant usage, select on sequence public.bv_activity_id_seq to service_role;
create policy bv_profile_read on public.bv_profiles for select to authenticated using ((select bv_private.member_role()) is not null);
...
create policy bv_request_read on public.bv_requests for select to authenticated using ((select bv_private.member_role())='ceo' or ((select bv_private.member_role()) is not null and created_by=(select auth.uid())));
create policy bv_activity_read on public.bv_activity for select to authenticated using (bv_private.can_read_task(task_id));

create function bv_private.action(p_action text,p jsonb) returns jsonb language plpgsql security definer set search_path='' as $$
declare
...
revoke all on function bv_private.action(text,jsonb) from public;
grant execute on function bv_private.action(text,jsonb) to authenticated;
create function public.bv_action(p_action text,p jsonb) returns jsonb language sql security invoker set search_path='' as $$ select bv_private.action(p_action,p) $$;
...
create function public.bv_state() returns jsonb language sql security invoker set search_path='' as $$
...
alter publication supabase_realtime add table public.bv_profiles,public.bv_tasks,public.bv_assignments,public.bv_requests,public.bv_activity;
